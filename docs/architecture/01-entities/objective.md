...
## Purpose
- Represents a named, trackable coaching goal for the current training block
...
type Objective = {
  ...
  training_block_id: string         // UUID, FK → TrainingBlock; scoped to block
...
- Objectives are scoped to a `training_block_id`. When a block closes, objectives for that block are not carried to the new block — new objectives are seeded.
- `ObjectiveUpdate` is append-only. Updates accumulate as a history.
- A maximum of 5 active objectives per block. New objectives supersede old ones when the limit is reached.
