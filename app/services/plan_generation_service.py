"""PlanGenerationService — atomic plan generation for Phase-1.4 modes.

This is the single owner of plan generation for ``race_event`` and
``target_performance`` goals. ``fitness_improvement``, ``maintenance``,
and ``recovery`` modes are out of scope for this sub-phase and the
service raises ``InvalidGoalTypeError`` when any of those is supplied.
The Coder Handoff Notes for Phase-1.4 are explicit: the service is
**pure Python — no LLM, no external API calls**. Deterministic
templates drive both phase allocation and per-week session synthesis.

Atomic transaction:

    PlanGenerationService.generate_plan runs every write through the
    per-entity repositories on a single shared ``AsyncSession`` and
    commits exactly once at the end of the method. Raising any
    exception before the commit causes SQLAlchemy to roll back the
    session automatically.

    The transaction covers:
      * supersession of the previous active plan (status →
        superseded + superseded_at stamp);
      * the new TrainingPlan row;
      * all WeeklyPlans with WeeklySessions;
      * all PlannedSession rows (one per WeeklySession);
      * all Checkpoint rows (one per flagged session);
      * the ``training_plan_generated`` event/outbox rows (ADR-004).

Invariant guarantees (each enforced by a dedicated helper):

    * One active plan per TrainingGoal at any time — supersession
      moves the previous plan to ``superseded`` in the same
      transaction as the new plan's creation.
    * Old plans are never deleted — ``superseded_at`` is the only
      mutation on an inactive plan.
    * ``phases`` are non-overlapping and ordered; the combined date
      range covers from the plan start date to
      ``TrainingGoal.goal_event_date`` without gaps.
    * PlannedSession records cover the full duration to the goal
      event with no gaps.
    * No two consecutive quality sessions unless they share a
      ``block_id``.
    * Long runs are always followed by a rest or recovery_run.
    * Threshold and VO2max sessions are sandwiched between easy or
      rest days.
    * WeeklyPlan sessions array is immutable once
      ``status = active`` — for Phase 1.4 we always create them with
      ``status = synthesised``.
    * One WeeklyPlan per ``(training_plan_id, week_number)`` (DB
      unique constraint).
    * ``PlannedSession.training_plan_id`` is denormalised —
      staleness is documented; queries MUST join through WeeklyPlan.
    * One Checkpoint per PlannedSession; PlannedSession's
      ``checkpoint_type`` / ``checkpoint_metric`` are set together.
    * Checkpoints cannot be created retroactively — only via
      synthesis.
    * ``strategic_rationale`` is set only for race_event and
      target_performance modes.
    * ``training_plan_generated`` event fires via the transactional
      outbox inside the same DB transaction.

Integration with onboarding:

    ``OnboardingService.complete_onboarding`` accepts an optional
    ``PlanGenerationService`` and invokes ``generate_plan`` at the end
    of the onboarding transaction, before commit. The dependency is
    forwarded from the FastAPI dependency factory so the API can
    construct the service for both routes.
"""
