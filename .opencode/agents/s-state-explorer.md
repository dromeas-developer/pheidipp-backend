...
description: >-
  ...
  p-implementation-architect, p-implementation-resolver, p-implementation-validator,
...
look like?" questions (that is `s-code-explorer`'s job).
...
what a specific file looks like, they should invoke `s-code-explorer`
...
7. **Do not resolve file content beyond what is needed for registry
   ...
   should invoke `s-code-explorer` after your brief.

## What You Do Not Do

* Do not resolve file content, method implementations, or business logic
  — that is `s-code-explorer`'s job
...
## Relationship to s-code-explorer

You and `s-code-explorer` are complementary, not overlapping:
...
| "What does `process()` do and how is it implemented?" | **s-code-explorer** |
| "What is the full method signature of `process()`?" | **s-code-explorer** |

**Calling convention:** Call State Explorer first (cheap registry check).
If the caller needs implementation details, call `s-code-explorer` after
