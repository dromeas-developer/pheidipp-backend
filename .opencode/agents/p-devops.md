...
Use `find_files` to locate `tests/test_manifest.yaml`.
Use `get_files` to read it.
If missing → STOP. Report MISSING_TEST_MANIFEST. Do not run builds.
...
If test execution fails, classify the failure before stopping.

DevOps MAY edit ONLY test infrastructure files:
* `tests/conftest.py`
* `tests/fixtures/**`
* `tests/helpers/**`
* `tests/utils/**`
...
* `tests/test_*.py`
* `tests/test_manifest.yaml` (except validation fields below)
* application source files

Remediation is allowed ONLY if the failure is caused by:
* test framework configuration
* database connection lifecycle
* fixture setup
* transaction visibility
* async/session binding
* test environment bootstrap

Examples:
* broken `AsyncSession` lifecycle
* missing metadata/model registration for test discovery
* schema reflection issues
* test database wiring

After remediation:
1. Record modified files and reason
2. Re-run `bash scripts/run-tests.sh` ONCE using the same scope
3. Use the second result as authoritative

If the retry still fails → STOP and continue with normal failure handling.

Do NOT weaken assertions, remove tests, mark tests skipped, or reduce
