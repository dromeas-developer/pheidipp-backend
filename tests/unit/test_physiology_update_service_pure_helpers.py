...
* ``_confidence_level(prior_weight)`` — maps a prior weight to a
  ``TwinConfidenceLevel`` value string using the 4.0 / 8.0 thresholds.
* ``_state_prior_weight(state)`` — extracts the prior weight from a
  ``PhysiologyParameterState`` dict.
* ``_compute_metric_confidence(physiology)`` — returns the per-metric
  confidence dict in ``TwinState`` shape.
* ``_detect_confidence_transitions(old, new)`` — returns the
  monotonic LOW→MEDIUM and MEDIUM→HIGH transitions between two
  per-metric confidence snapshots.
* ``_parse_iso_date(value)`` — parses the JSONB
  ``last_observation_date`` string.
* ``_coerce_observation_date(value)`` — coerces an observation date
  to ``datetime.date``.
* ``_source_value(source)`` — returns the ``MeasurementSource.value``
  string.
...
from app.services.physiology_update_service import (
    _coerce_observation_date,
    _compute_metric_confidence,
    _confidence_level,
...
# ---------------------------------------------------------------------------
# _confidence_level — prior weight → TwinConfidenceLevel mapping.
# ---------------------------------------------------------------------------


class TestConfidenceLevel:
    """``_confidence_level`` maps a prior weight to LOW / MEDIUM / HIGH
    ...
    def test_prior_weight_above_eight_returns_high(self) -> None:
        ...
        assert _confidence_level(8.0) == TwinConfidenceLevel.HIGH.value
        assert _confidence_level(10.0) == TwinConfidenceLevel.HIGH.value
        assert _confidence_level(100.0) == TwinConfidenceLevel.HIGH.value

    def test_prior_weight_between_four_and_eight_returns_medium(self) -> None:
        """``4.0 <= prior_weight < 8.0`` → ``"medium"``."""
        assert _confidence_level(4.0) == TwinConfidenceLevel.MEDIUM.value
        assert _confidence_level(5.0) == TwinConfidenceLevel.MEDIUM.value
        assert _confidence_level(7.99) == TwinConfidenceLevel.MEDIUM.value

    def test_prior_weight_below_four_returns_low(self) -> None:
        """``prior_weight < 4.0`` → ``"low"``."""
        assert _confidence_level(0.0) == TwinConfidenceLevel.LOW.value
        assert _confidence_level(1.0) == TwinConfidenceLevel.LOW.value
        assert _confidence_level(3.99) == TwinConfidenceLevel.LOW.value

    def test_none_prior_weight_returns_low(self) -> None:
        """``None`` prior weight (no evidence yet) → ``"low"``."""
        assert _confidence_level(None) == TwinConfidenceLevel.LOW.value

    def test_exact_threshold_eight_is_high(self) -> None:
        """The 8.0 threshold is inclusive (``>=``) — exactly 8.0 is HIGH."""
        assert _confidence_level(8.0) == "high"

    def test_exact_threshold_four_is_medium(self) -> None:
        """The 4.0 threshold is inclusive (``>=``) — exactly 4.0 is MEDIUM."""
        assert _confidence_level(4.0) == "medium"


# ---------------------------------------------------------------------------
# _state_prior_weight — extract prior weight from a state dict.
# ---------------------------------------------------------------------------


class TestStatePriorWeight:
    """``_state_prior_weight`` extracts the ``prior_weight`` from a
    ...
    def test_returns_prior_weight_from_state(self) -> None:
        ...
        assert _state_prior_weight(state) == 3.5

    def test_returns_none_for_none_state(self) -> None:
        """A ``None`` state returns ``None``."""
        assert _state_prior_weight(None) is None
    ...
    def test_returns_none_when_prior_weight_key_missing(self) -> None:
        ...
        assert _state_prior_weight(state) is None
    ...
    def test_returns_float_for_integer_prior_weight(self) -> None:
        ...
        result = _state_prior_weight(state)
        assert result == 4.0
        assert isinstance(result, float)


# ---------------------------------------------------------------------------
# _compute_metric_confidence — per-metric confidence dict.
# ---------------------------------------------------------------------------


class TestComputeMetricConfidence:
    """``_compute_metric_confidence`` returns a per-metric dict in
    ...
    def test_returns_seven_keys(self) -> None:
        ...
        result = _compute_metric_confidence(physiology)
...
# ---------------------------------------------------------------------------
# _detect_confidence_transitions — monotonic upward transitions.
# ---------------------------------------------------------------------------


class TestDetectConfidenceTransitions:
    """``_detect_confidence_transitions`` returns a per-metric dict of
    ...
    def test_low_to_medium_reported(self) -> None:
        ...
        old = {"lt2_hr": "low"}
        new = {"lt2_hr": "medium"}
        result = _detect_confidence_transitions(old, new)
...
# ---------------------------------------------------------------------------
# _parse_iso_date — JSONB last_observation_date parsing.
# ---------------------------------------------------------------------------


class TestParseIsoDate:
    """``_parse_iso_date`` parses the JSONB ``last_observation_date``
    string into a ``datetime.date``."""

    def test_bare_date_string(self) -> None:
        """A bare ``YYYY-MM-DD`` string parses to a date."""
        result = _parse_iso_date("2026-05-12")
    ...
    def test_unsupported_type_raises_type_error(self) -> None:
        ...
        with pytest.raises(TypeError, match="unsupported last_observation_date"):
            _parse_iso_date(12345)


# ---------------------------------------------------------------------------
# _coerce_observation_date — observation date coercion.
# ---------------------------------------------------------------------------


class TestCoerceObservationDate:
    """``_coerce_observation_date`` coerces an observation date value
    to ``datetime.date``."""

    def test_date_instance_returned_as_is(self) -> None:
        """A ``date`` instance is returned as-is."""
        d = date(2026, 6, 15)
        result = _coerce_observation_date(d)
        assert result is d

    def test_datetime_instance_truncated_to_date(self) -> None:
        """A ``datetime`` instance is truncated to its date component."""
        dt = datetime(2026, 6, 15, 14, 30, tzinfo=timezone.utc)
        result = _coerce_observation_date(dt)
    ...
    def test_unsupported_type_raises_type_error(self) -> None:
        ...
        with pytest.raises(TypeError, match="unsupported observation date"):
            _coerce_observation_date("2026-06-15")


# ---------------------------------------------------------------------------
# _source_value — MeasurementSource.value extraction.
# ---------------------------------------------------------------------------


class TestSourceValue:
    """``_source_value`` returns the ``MeasurementSource.value`` string
    for a source value."""

    def test_enum_member_returns_value_string(self) -> None:
        """A ``MeasurementSource`` enum member returns its ``.value``."""
        result = _source_value(MeasurementSource.TRAINING_HR_DEFLECTION)
    ...
    def test_pre_stringified_value_returned_as_is(self) -> None:
        ...
        result = _source_value("training_rr_inflection")
