...
class TestTrainingPreferencesEndpoints:
    """Tests for training preferences API endpoints."""

    @pytest.mark.asyncio
    async def test_create_training_preferences_endpoint_returns_201(
        self, client: AsyncClient, test_db_session
    ):
        """Verify successful creation response contract returns 201."""
        ...
        response = await client.post(
            f"/athletes/{athlete.id}/training-preferences",
            json=payload,
        )
        ...
        assert data["goal_event_type"] == "half_marathon"
        assert data["fitness_level"] == 5

    @pytest.mark.asyncio
    async def test_get_training_preferences_endpoint_returns_latest_record(
        self, client: AsyncClient, test_db_session
    ):
        """Verify endpoint returns active preferences version (latest record)."""
        from app.models.athlete import Athlete
        from app.models.enums import AthleteStatus
        ...
        # Create multiple versions of training preferences
        old_pref = TrainingPreferences(
            id=uuid.uuid4(),
        ...
        test_db_session.add(old_pref)
        test_db_session.add(new_pref)
        await test_db_session.commit()

        # Get active (latest) training preferences
        response = await client.get(
            f"/athletes/{athlete.id}/training-preferences/active",
        )

        assert response.status_code == 200
        data = response.json()
        # Should return the latest one (new_pref)
        assert data["goal_type"] == "race"
        assert data["fitness_level"] == 5
