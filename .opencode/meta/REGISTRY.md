...
| p-implementation-validator | Post-implementation audit | nvidia/z-ai/glm-5.2 | p-index-health-guard, p-state-explorer, p-contract-verifier, p-code-structure-explorer | git-session-delta, no-silent-deviations, stack-truth-conformance, validation-classification-and-report, todowrite-discipline | get_files, find_files, grep_files, search_codebase, search_symbols, get_phase_context | task(index-health-guard/state-explorer/contract-verifier/structure-explorer), edit/write(allow), bash(deny), read(deny), todowrite(allow) |
