---
name: implementation-handoff-blocks
description: >
  ...
  1-6). Contains the full spec for Context Needed tiers, Batch Success
  Criteria, and architecture documentation handoff conventions. In
  Resolution Mode, also triggered when a plan update touches a BRD's
  Context Needed or Batch Success Criteria — load it to update those
  blocks per this spec.
---
...
Completion Verification protocol already invokes `p-documentation`
...
`p-vision-and-architect-author` (per its prompt: "You are the only
...
`batch-N-architecture.md` alongside the BRD — follow Template 4 in the
`implementation-plan-templates` skill. The coder never loads it; it is
for `p-vision-and-architect-author` only.
...
p-vision-and-architect-author") so the batch's scope boundary is
explicit.
