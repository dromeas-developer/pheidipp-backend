...
Migration generation is delegated to `s-alembic` — the coder does
NOT write migration files, run `db-revision*.sh`, or load the
`infrastructure-reference` skill. In both Batch Mode and Fix Mode,
after all code changes are complete, invoke `s-alembic` with the
`generate` operation (mode: `auto`). s-alembic checks for ORM drift,
generates the migration if needed, applies TimescaleDB hypertable
augmentation, and upgrades the test DB.

If no ORM changes were introduced, s-alembic no-ops and returns
"No migration needed."

Migration verification (`pending-changes-check`) and execution
(`apply-test`, `apply-prod`) are owned by `s-alembic` as well,
invoked by `p-test-runner` and `p-devops` respectively.

A validator or devops finding that itself concerns migration or table
scope is never routed to you in the first place — so this rule only
fires when an otherwise in-scope fix happens to touch a model.
