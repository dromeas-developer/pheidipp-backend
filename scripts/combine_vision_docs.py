...
def parse_reading_order(vision_dir: Path) -> list[Path]:
    ...
    if not reading_order_file.exists():
        print(
            f"Warning: {reading_order_file} not found, falling back to default ordering"
        )
        return []
...
def combine_vision_docs(output_file: str = "VISION_COMBINED.md") -> None:
    ...
    if not md_files:
        print(
            "Error: Could not extract file list from reading-order.md or file not found"
        )
        return
            ...
            try:
                with open(md_file, "r", encoding="utf-8") as infile:
                    ...
                    if md_file.parent == vision_dir:
                        ...
                        section_title = (
                            str(relative_path).replace(".md", "").replace("/", " > ")
                        )
