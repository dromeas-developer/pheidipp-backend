"""PhysiologyUpdateService — Bayesian update of AthletePhysiology posterior state.

Implements the Phase-2.3-P2 contract from
``docs/implementation/phase-2/phase-2-3-p2-physiology-update.md``
and the Bayesian update formula in
``docs/architecture/02-computations/physiology-update.md``.

Module surface:

* :func:`bayesian_update` — pure function that applies the
  architecture's posterior-mean / posterior-uncertainty / prior
  decay formula to a single ``PhysiologyParameterState`` and a
  single observation. Module-level so unit tests can exercise it
  directly without a session.
* :func:`init_null_parameter_state` — pure function that
...
* :class:`PhysiologyUpdateService` — the orchestrator that loads
  the athlete's posterior state, dedups duplicate observations
  (writing the ``PhysiologyMeasurement`` audit row but skipping
  the Bayesian update and event contribution), routes each
  ``ThresholdObservation`` to the right parameter, runs the
  Bayesian update, persists the ``PhysiologyMeasurement`` audit
  record, mutates the ``AthletePhysiology`` JSONB columns in
  place (with ``flag_modified`` so SQLAlchemy persists the
  change), detects posterior shifts above the 1-unit threshold,
  detects monotonic confidence transitions, and fires the
  ``physiology_updated`` event via :class:`EventPublisher` when
  at least one parameter shifted.
* :class:`PhysiologyUpdateResult` — return-value dataclass
  consumed by the worker task that appends a fresh ``TwinState``
  using the returned ``metric_confidence`` and surfaces
  ``confidence_transitions`` to the
  ``TwinRecalibrationService`` for ``twin_confidence_upgraded``
  event firing.
* :class:`MissingAthletePhysiologyError` — raised when the
  athlete has no ``AthletePhysiology`` row (data-integrity
  failure; the onboarding bootstrap always creates one).

Invariants codified here, copied from the architecture corpus:

* The service mutates ``AthletePhysiology`` JSONB columns in
  place — the row is mutable, history lives on
  ``PhysiologyMeasurement`` and inline ``TwinState`` snapshots.
* ``PhysiologyMeasurement`` is ALWAYS written, even when the
  posterior does not shift or the observation is a duplicate
  — the table is the complete observation history. The
  ``> 1 unit shift`` gate applies ONLY to the
  ``physiology_updated`` event, not to the measurement write.
* The ``physiology_updated`` event is fired ONLY when at least
  one parameter shifted by > 1 unit — minor fluctuations do
  not produce events.
* The event is written to the transactional outbox
  (``SystemEvent`` + ``SystemEventOutbox``) in the same
  transaction as the ``AthletePhysiology`` update — see
  ADR-004 / ``EventPublisher``.
* ``flag_modified`` must be called on every JSONB column that
  was touched — SQLAlchemy does not auto-detect in-place
  mutations to JSONB.
