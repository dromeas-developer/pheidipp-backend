...
| p-code-structure-explorer | AST-based structure resolver | deepseek-v4-flash-free | none | none | get_module_context, get_class_context, get_function_context, list_imports, get_module_deps, get_importers, search_symbols, multi_code_query | task(deny all), skill(deny), bash(deny) — invoked by p-coder, p-implementation-architect, p-implementation-validator |
| p-state-explorer | Codebase registry resolver | deepseek-v4-flash-free | none | retrieval-patterns | search_symbols, search_codebase, grep_files, find_files, get_files, get_code_for_entity, multi_code_query | task(deny all), bash(deny) |
