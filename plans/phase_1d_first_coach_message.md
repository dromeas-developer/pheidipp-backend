...
### 8 — Update `app/config.py` [MODIFY]
- Objective: Add Anthropic API key configuration.
- Actions:
  - Add `ANTHROPIC_API_KEY: str = Field(default="", env="ANTHROPIC_API_KEY")` to `Settings`.

### 9 — Create `app/core/llm.py` [CREATE]
- Objective: Cached Anthropic async client singleton.
- Actions:
  - Import `AsyncAnthropic` from `anthropic`.
  - Import `lru_cache` from `functools`.
  - Import `settings` from `app.config`.
  - Define `@lru_cache(maxsize=1)` function `get_anthropic_client()` returning `AsyncAnthropic(api_key=settings.ANTHROPIC_API_KEY)`.

### 10 — Create `app/core/telemetry.py` [CREATE]
...
### 11 — Create `app/agents/prompts/__init__.py` [CREATE]
- Objective: Package init for prompts module.
- Actions:
  - Create empty `__init__.py`.

### 12 — Create `app/agents/prompts/registry.py` [CREATE]
...
### 13 — Create `app/agents/prompts/first_message_v1.py` [CREATE]
...
### 14 — Create `app/repositories/coach_message_repository.py` [CREATE]
...
### 15 — Update `app/repositories/__init__.py` [MODIFY]
...
### 16 — Update `app/core/unit_of_work.py` [MODIFY]
...
### 17 — Create `app/services/coach_message_service.py` [CREATE]
...
### 18 — Create `app/services/first_message_brief_builder.py` [CREATE]
...
- Actions:
  ...
  - **Correction from draft**: `first_name` must be read from `profile.first_name`, not `athlete.first_name` (Athlete model has no first_name field).
...
### 19 — Update `app/services/__init__.py` [MODIFY]
...
### 20 — Create `app/agents/first_message_agent.py` [CREATE]
- Objective: LLM call wrapper with telemetry and validation.
- Actions:
  - Import `get_anthropic_client`, `PromptRegistry`, `GenerationEvent`, `log_generation_event`, `GenerationOutcome`.
  ...
  - Call `client.messages.create(model="claude-sonnet-4-6", max_tokens=..., system=..., messages=[...])`.
  - Track latency with `time.monotonic()`.
  - On success: validate response has at least 2 paragraph breaks (`\n\n`), else mark `MALFORMED` and raise.
  - On `APITimeoutError`: mark `TIMEOUT` and raise.
  - On `APIStatusError`: mark `RATE_LIMITED` if 429, else `PROVIDER_ERROR`, and raise.
  - On unexpected exception: mark `INTERNAL_ERROR` and raise.
  - Always call `log_generation_event` before returning or raising.
  - On success return `(content: str, generation_metadata: dict)` where metadata includes model, prompt_version, brief_version, outcome, input_tokens, output_tokens, latency_ms, stop_reason, data_tier, confidence_level, context_budget.

---

## Tasks

### 21 — Create `app/tasks/__init__.py` [CREATE]
- Objective: Package init for background tasks.
- Actions:
  - Create empty `__init__.py`.

### 22 — Create `app/tasks/first_message_task.py` [CREATE]
...
### 23 — Update `app/api/dependencies/services.py` [MODIFY]
...
### 24 — Create `app/api/routes/coach_messages.py` [CREATE]
...
### 25 — Update `app/api/routes/athletes.py` [MODIFY]
...
- Actions:
  ...
  - In `onboard_athlete` endpoint:
    ...
    - **Important**: The background task must be added **after** the transaction commits (outside the `async with` block) so the twin state and training block are visible to the independent session in the task.
...
### 27 — Alembic migration for `coach_messages` table [MIGRATION]
...
1. **Repository pattern**: `CoachMessageRepository` now extends `BaseRepository[CoachMessage]` and uses `session.flush()` (not `commit`) to align with UoW transaction ownership.
2. **UnitOfWork**: Added `coach_messages` repository to `_repos` dict.
3. **Athlete.first_name**: The `Athlete` model has no `first_name` field — it lives on `AthleteProfile`. The brief builder reads `profile.first_name`.
4. **Background task session**: The task creates its own `AsyncSessionLocal()` session instead of reusing the request session, since it runs after the request completes.
5. **Background task trigger**: `background_tasks.add_task()` is called **outside** the `UnitOfWork` block in `onboard_athlete`, ensuring the transaction is committed before generation starts.
6. **Onboarding status**: Uses `UnitOfWork` to check `has_first_message` instead of a bare repository call.
7. **Config**: Added `ANTHROPIC_API_KEY` to `Settings`.
8. **Route registration**: Added `coach_messages` router to `main.py`.
9. **Service dependencies**: Added `get_coach_message_service` factory.
