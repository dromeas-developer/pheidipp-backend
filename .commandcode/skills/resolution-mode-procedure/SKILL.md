...
description: >
  ...
  from p-implementation-validator or p-devops routed to implementation-architect,
...
Only act on what is routed to `p-implementation-architect`. Everything else in the
same report is real, correctly reported, and not yours.
...
from the devops report's plan id. Fetch it via `read_file` — once,
...
the coder started) — edit the affected file directly using the `edit_file`
...
* **no shipped batch BRD was edited in place** — if a resolution touched
  ...
  against. If you find yourself about to `edit_file` a shipped batch BRD, STOP
...
the `write_file` tool. Follow the Resolution Report Format below exactly.
