"""SignalCleaningService — execute the 7-step signal-cleaning pipeline.

Implements Phase-2.2 of the implementation plan
``docs/implementation/phase-2/phase-2-2-p1-signal-cleaning.md``.

This service is the single owner of step-1–4 logic from
``docs/architecture/02-computations/signal-cleaning.md``:

* Step 1 — Artifact removal (two-stage RR: hard bound then ±20% rolling-median
  deviation filter)
* Step 2 — Smoothing / filtering
* Step 3 — Derived metrics (Gen-1 population GAP)
* Step 4 — Rolling features (30/60/120-second means + variability index)

Steps 5–7 (changepoint detection, state inference, segment alignment)
are deferred to the segmentation phases per the sub-phase
``Simplifications deferred`` note. The fixed pipeline order is
preserved by emitting the steps as private methods whose order is
enforced by the call sequence in :meth:`clean` — there is no
``_run_step_N`` dispatcher that could be re-ordered.

Public surface:

* :meth:`SignalCleaningService.clean` — single async entry point.
  Returns a :class:`CleaningResult` indicating whether a
  ``RawSensorStream`` row was created.

Pipeline invariants (codified here, copied from the architecture
corpus):

* Steps run in fixed order 1→4. No step may be skipped or reordered.
* Null propagation: artifact-removed nulls propagate through
  smoothing. A channel with > 80% null values after artifact removal
  is marked unavailable in :class:`AvailableChannels`.
* Resampling: FIT files vary in recording rate; the pipeline
  resamples to a uniform 1 Hz time series before step 1.
* If the pipeline produces a stream shorter than 5 minutes of
  non-null HR data, ``RawSensorStream`` is not created and
  segmentation is skipped.
* The cleaned RR series excludes samples deviating more than ±20%
  from their trailing rolling median (sub-phase Exit Gate
  verbatim: "Cleaned streams pass artifact validation thresholds
  (RR values within ±20% of rolling median retained)"). The
  two-stage RR artifact removal — hard bound then ±20% rolling
  median deviation filter — is the cleaned-stream contract that
  the downstream ``ThresholdDetectionService`` HRV-inflection step
  1 ("values outside ±20% of rolling median removed") consumes.

Smoothing fallback note:

* HR exponential moving average is implemented inline (no
  dependency).
* Power and pace use ``scipy.signal.savgol_filter`` (window=7,
  polynomial=3) per the spec. If scipy is unavailable at runtime the
  service raises :class:`SignalCleaningError` at construction time
  rather than silently degrading — the architecture specifies
  Savitzky-Golay and silent fallback would violate the
  ``Real Signals, Not Assumptions`` principle.
"""
