...
causes may belong to `p-coder-fix-mode`, others to `p-test-architect`, others may
...
Before resolving test execution scope, verify the code index is fresh by invoking `s-index-health-guard`:
...
Report as RC: Category `Implementation`, Owner `p-coder-fix-mode`, Confidence
...
Category `Implementation`, Owner `p-coder-fix-mode`, Confidence `Confirmed`,
...
`p-implementation-resolver`, Confidence per your certainty) instead of guessing.
...
Owner `p-coder-fix-mode`, unless the error is clearly an extension/environment
...
- **Non-empty `upgrade()` body** → the migration applied in Step 3 does
  ...
  `p-coder-fix-mode`, Confidence `Confirmed` (the diff itself is direct proof),
...
2. Invoke `s-manifest-manager` to handle the index.yaml update (split check
...
1. Invoke `s-manifest-manager` to handle the release promotion (move,
   collapse check):
...
cause is. Escalate to `p-implementation-resolver` instead only if the divergence itself
...
Category `Implementation`, Owner `p-coder-fix-mode`; a crash traceable to
