...
description: >-
  Maintains per-folder README.md documentation for app/ and tests/.
   Four modes, never combined: Incremental (invoked by p-coder-batch-mode or
...
**From p-coder-batch-mode** (app/ READMEs):
...
* A BRD path → Incremental (p-coder-batch-mode)
...
Invoked by `p-coder-batch-mode` or `p-test-architect` at the end of every batch
implementation or test generation. Determine the caller from what was
provided: a BRD path → p-coder-batch-mode; a manifest path → p-test-architect.

**Procedure (p-coder-batch-mode — app/ READMEs):**
