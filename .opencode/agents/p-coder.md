...
Migration application and build verification are owned by `p-devops`.
...
Migration generation and execution belong to `p-devops`.

Implement ORM model and schema changes only. Do not generate, modify, or apply
migration files. Do not run `alembic` or `bash scripts/db-revision.sh`.
