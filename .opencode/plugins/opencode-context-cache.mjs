 ...
 *
 * Gating (plugin-specific flag):
 * - The cache key (promptCacheKey + session headers) is only injected when
 *   the provider or model opts in via `contextCache: true` in opencode.jsonc
 *   (provider-level or model-level `options`). This flag is independent of
 *   opencode's native `setCacheKey` — set `contextCache: true` to use this
 *   plugin, `setCacheKey: true` to use opencode native caching, or both to
 *   compare (the last-applied value wins for the promptCacheKey field).
 *   Providers that reject the `promptCacheKey` request field (e.g. NVIDIA
 *   NIM through litellm-proxy) must NOT enable either flag.
 */
...
class CacheKeyApplier {
  ...
  isEnabledForProvider(input) {
    const providerOptedIn = input?.provider?.options?.contextCache === true;
    const modelOptedIn = input?.model?.options?.contextCache === true;
    return providerOptedIn || modelOptedIn;
  }
...
class ContextCachePluginRuntime {
  ...
  handleChatParams(input, output) {
    ...
    // Only inject the cache key when the provider/model opts in via the
    // plugin-specific `contextCache: true` flag in opencode.jsonc — this is
    // independent of opencode's native `setCacheKey`. Providers that reject
    // the `promptCacheKey` request field (e.g. NVIDIA NIM via litellm-proxy)
    // must not enable either flag.
    if (!this.keyApplier.isEnabledForProvider(input)) {
      this.logger.log("Cache key disabled for this provider/model (contextCache not set); skipping injection");
      return;
    }
